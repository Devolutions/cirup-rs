...
struct Cli {
    ...
    #[arg(short = 'c', long = "config", global = true, help = "Sets the configuration file to use")]
    config: Option<String>,

    #[arg(short = 'o', long = "old-commit", global = true, help = "a git hash specifying the old commit")]
    old_commit: Option<String>,

    #[arg(short = 'n', long = "new-commit", global = true, requires = "old_commit", help = "a git hash specifying the new commit")]
...
enum Commands {
    ...
    #[command(name = "vcs-log", about = "show the version control history of the source language, newest first. [config] is required.")]
    VcsLog {
        #[arg(short = 'l', long = "limit", default_value_t = 0, help = "limit the number of results returned")]
        limit: u32,
    ...
    #[command(name = "vcs-diff", about = "diff two commits of the source language. [config] is required. [old-commit] is required.")]
    VcsDiff,
    ...
    #[command(name = "file-convert", about = "convert [file] to another type. possible extensions are .json, .resx and .restext")]
    FileConvert {
        file: String,
        output: String,
    },

    #[command(name = "file-sort", about = "sort [file] by key name. possible extensions are .json, .resx and .restext")]
    FileSort {
        file: String,
        output: Option<String>,
    },

    #[command(name = "file-diff", about = "output keys that have values in [file1] but not in [file2]. useful for finding missing translations.")]
    FileDiff {
        file1: String,
        file2: String,
        output: Option<String>,
    ...
    #[command(name = "file-intersect", about = "output the intersection of values from [file1] and [file2]")]
    FileIntersect {
        file1: String,
        file2: String,
        output: Option<String>,
    ...
    #[command(name = "file-subtract", about = "outputs values from [file1] that do not exist in [file2]")]
    FileSubtract {
        file1: String,
        file2: String,
        output: Option<String>,
    ...
    #[command(name = "diff-with-base", about = "output keys that have values in [new] but not in [old] with the value in [base]")]
    DiffWithBase {
        old: String,
        new: String,
        base: String,
